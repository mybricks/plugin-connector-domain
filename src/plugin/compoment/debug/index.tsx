...
export default function Debug({formModel, validate}: any) {
	const [schema, setSchema] = useState(formModel.resultSchema);
	const allDataRef = useRef<any>();
	const [errorInfo, setError] = useState('');
	const [params, setParams] = useState(formModel.params);
	const [edit, setEdit] = useState(false);
	const [context] = useState({formModel, editNowId: void 0});
	formModel.params = formModel.params || {
		type: 'root',
		name: 'root',
		children: [],
	};
	...
	return (
		<>
			<FormItem label='请求参数'>
				<ParamsEdit value={formModel.params} ctx={context} onChange={onParamsChange}/>
			</FormItem>
			<FormItem>
				<Params onDebugClick={onDebugClick} ctx={context} params={params}/>
			</FormItem>
			{edit ? (
				<>
					<FormItem label='返回数据'>
